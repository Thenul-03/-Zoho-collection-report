...
async function buildPdf(rows, totals, salesReceiptRows, salesReceiptTotal, from, to) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const signatureLabelY = signatureLineY + 6;
...
async function buildWorkbook(rows, totals, salesReceiptRows, salesReceiptTotal, from, to) {
  ...
  [[1, 4], [6, 9], [11, 13]].forEach(([start, end]) => {
    for (let column = start; column <= end; column++) {
      sheet.getCell(rowIndex, column).border = { bottom: signatureLine };
    }
  });
  sheet.getRow(rowIndex).height = 24;
  sheet.mergeCells(rowIndex + 1, 1, rowIndex + 1, 4); sheet.getCell(rowIndex + 1, 1).value = 'REPORT GENERATED BY';
  sheet.mergeCells(rowIndex + 1, 6, rowIndex + 1, 9); sheet.getCell(rowIndex + 1, 6).value = 'REPORT CHECKED BY';
  sheet.mergeCells(rowIndex + 1, 11, rowIndex + 1, 13); sheet.getCell(rowIndex + 1, 11).value = 'AUTHORIZED BY';
  [1, 6, 11].forEach((column) => { sheet.getCell(rowIndex + 1, column).font = { bold: true }; sheet.getCell(rowIndex + 1, column).alignment = { horizontal: 'center' }; });
