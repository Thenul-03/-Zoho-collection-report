  ...
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(1.5);
      ...
      ['REPORT GENERATED BY', 'REPORT CHECKED BY', 'AUTHORIZED BY'].forEach((label, index) => {
        ...
        doc.fontSize(9).font('Helvetica-Bold').text(label, x, doc.y, { width: signatureWidth, align: 'center' });
        doc.moveTo(x + 8, signatureY).dash(2, { space: 2 }).lineTo(x + signatureWidth - 8, signatureY).stroke('#555').undash();
      });
    doc.end();
  });
}
